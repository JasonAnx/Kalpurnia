...
function processQuery($termArray){
    ...
    foreach ($resultArray as $key => $value) {
        ...
        $finalResults[$counter] = "<li class=' '  style='display: list-item;'><h3>".$urls[$key]['title']."</h3><p class='flow-text'><a href='".$urls[$key]['url']."' target='_blank'>".$urls[$key]['url']."</a></p><br><p class='flow-text'>".$sntcs['count']." sentences expressing emotion found, ".$sntcs    ['pos']." positve and ".$sntcs['neg']." negative. </p></li>";
        $counter++;
    }
    return $finalResults;
}
